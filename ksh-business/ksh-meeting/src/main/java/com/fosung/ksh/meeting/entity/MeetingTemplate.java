...
@Data
public class MeetingTemplate extends AppJpaBaseEntity implements AppJpaSoftDelEntity {
    /**
     * 好视通会议室ID
     */
     ...
     * 模板类型
     */
    @ApiModelProperty("模板类型，GENERAL普通会议室, CYCLE循环会议室")
    @Enumerated(EnumType.STRING)
     ...
     */
    @Transient
    private MeetingTemplateCycle cycle;


    @ApiModelProperty("会议是否签到 sign 1 是 0否")
    @Column(name = "sign")
    @Enumerated(EnumType.STRING)
    ...
    @ApiModelProperty("是否补签 1补签 0不补签")
